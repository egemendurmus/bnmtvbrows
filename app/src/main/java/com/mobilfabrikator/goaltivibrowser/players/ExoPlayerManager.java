...
public class ExoPlayerManager {
    ...
    DefaultDataSourceFactory dataSourceFactory,dataSourceFactoryVid;
    String uriString = "";
    ...
    Context context;
    private MediaCodecRenderer videoRenderer;
    ...
    public ExoPlayerManager(Context mContext) {
        ...
        dataSourceFactory = new DefaultDataSourceFactory(mContext, Util.getUserAgent(mContext, "Nexa"), BANDWIDTH_METER);
        dataSourceFactoryVid = new DefaultDataSourceFactory(mContext, getDefaultUserAgent(), BANDWIDTH_METER);

        final MediaSource videoSource = new ProgressiveMediaSource.Factory(dataSourceFactory).createMediaSource(mp4VideoUri);
    ...
    public void playStream(String urlToPlay) {
        ...
        if (uriString.toUpperCase().contains("M3U8")) {
          //  videoSource = new HlsMediaSource(mp4VideoUri, dataSourceFactory, null, null);
            ...
            //  null, null);
            videoSource = new ProgressiveMediaSource.Factory(dataSourceFactoryVid).createMediaSource(mp4VideoUri);
        }
    ...
    public void playVideoStream(String urlToPlay) {

        uriString = urlToPlay;
        Uri mp4VideoUri = Uri.parse(uriString);
        MediaSource videoSource = null;
        String[] filenameArray = urlToPlay.split("\\.");
        //   MediaSource mediaSource = new ProgressiveMediaSource.Factory(dataSourceFactoryVid).createMediaSource(Uri.parse(urlToPlay));
        // Prepare the player with the source.

        MediaSource mediaSource = new ExtractorMediaSource.Factory(dataSourceFactory).
                createMediaSource(Uri.parse(urlToPlay));
